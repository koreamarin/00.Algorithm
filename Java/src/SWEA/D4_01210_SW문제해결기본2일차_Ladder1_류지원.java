...
public class D4_01210_SW문제해결기본2일차_Ladder1_류지원 {
    public static void main(String[] args) throws IOException {
        ...
        for(int t=1;t<11;t++) {		// 테스트케이스만큼 반복하는 반복문 실행
            ...
            int i=98;
            // i를 98에서 0까지 sweep할 예정. 따라서 i가 0이면 종료되는 반복문
            while(i!=0) {
                if(j>=1 && arr[i][j-1]==1) {	        // 만약 왼쪽 방향이 1이라면 실행
                    while(j>=1 &&arr[i][j-1]==1) {      // 왼쪽이 1이 아닐때까지 실행
                        j--;                            // j에서 1을 빼서 왼쪽으로 이동
                    }
                    i--;                                // 왼쪽으로 모두 이동하는 반복문을 빠져나왔다면 i에서 1을 빼서 위쪽으로 이동
                }
                else if(j<=98 && arr[i][j+1]==1){       // 오른쪽이 0일때까지 계속 오른쪽으로 이동
                    while(j<=98 && arr[i][j+1]==1) {    // 오른쪽이 1이 아닐때까지 실행
                        j++;                            // j에서 1을 더해서 오른쪽으로 이동
                    }
                    i--;                                // 왼쪽으로 모두 이동하는 반복문을 빠져나왔다면 i에서 1을 빼서 위쪽으로 이동
                }
                else i--;                               // 위 해당사항(왼쪽이나 오른쪽으로 이동)에 해당하지 않을 시 그냥 위로 이동.
            }
            System.out.println("#" + T + " " + j);      // j를 출력. j는 최종 열 위치.
        }
    }
}
